...
COPY . /go/src/git.merostm.com/go-sse-quote
WORKDIR /go/src/git.merostm.com/go-sse-quote
...
RUN mkdir ~/.ssh && umask 0077 && \
    ...
    go get -d -v && \
    go build -o /go/bin/go_sse_quote_service && \
    rm ~/.ssh/id_rsa

CMD ["/go/bin/go_sse_quote_service"]